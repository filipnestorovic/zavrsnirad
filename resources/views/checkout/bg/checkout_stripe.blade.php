...
<div class="wrapper">
    <main>
        ...
        <form action="{{$orderRoute}}" method="POST" id="payment-form">
            {{ csrf_field() }}
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="orderst-form hidden">
                                ...
                                <div class="step-title js-customer-title">
                                    <h2 class="title">
                                        Korak 3: <strong>Način plaćanja</strong>
                                    </h2>
                                </div>

                                <div class="" style="font-size: 20px;">
                                    <ul class="nav nav-tabs nav-fill" id="myTab" role="tablist">
                                        @if($product->enabledCOD)
                                            <li class="nav-item">
                                                <a class="nav-link active" id="cash-tab" data-toggle="tab"
                                                   href="#cashDiv" role="tab" aria-controls="cashDiv">Cash on delivery</a>
                                            </li>
                                        @endif
                                        @if($product->enabledStripe)
                                            <li class="nav-item">
                                                <a class="nav-link" id="stripe-tab" data-toggle="tab" href="#stripeDiv"
                                                   role="tab" aria-controls="stripeDiv">
                                                    <img src="{{ asset('/') }}shared_files/stripe.png" alt="stripe_icon"
                                                         height="30px"/>
                                                </a>
                                            </li>
                                        @endif
                                    ...
                                    <div class="tab-content" id="myTabContent">
                                        @if($product->enabledCOD)
                                            <div class="tab-pane fade show active" id="cashDiv" role="tabpanel" aria-labelledby="cash-tab">
                                        ...
                                        @endif
                                        @if($product->enabledStripe)
                                            <div class="tab-pane fade" id="stripeDiv" role="tabpanel" aria-labelledby="stripe-tab">
                                                <div>
                                                    <form action="" method="post" id="payment-form">
                                                        @csrf
                                                        <div class="m-3">
                                                            <div id="fetchLoader" class="text-center align-middle">
                                                                <img src="{{ asset('/') }}shared_files/circle_loading.gif"
                                                                     width="80px"
                                                                     alt="fetchLoader">
                                                            </div>
                                                            <div id="payment-element" class="d-none" style="margin-top: 250px;">

                                                            </div>
                                                        </div>
                                                        <div class="w_button btn-order-now m-3">
                                                            <button type="submit" id="payment-button" class="d-none">
                                                                <span>ZAVRŠI KUPOVINU</span>
                                                            </button>
                                                        </div>
...
<script>
    ...
    let elements = '';
    let formMounted = 0;
    let selectedQuantity = $('input[name="quantity"]:checked').attr('data-product-quantity');
    let variation_id = {{$variation_id}};
    let paymentIntentId;

    $(document).ready(function () {
        @if(($product->enabledCOD == 0) && ($product->enabledStripe == 1))
            $('#stripe-tab').click();
        @endif
    });

    $('#stripe-tab').on('click',function () {
        if(!formMounted) {
            fetchStripeForm();
        }
    });

    async function fetchStripeForm() {
        const response = await fetch('/api/stripe/createPaymentIntent/' + variation_id + '/' + selectedQuantity);
    ...
    $('#payment-button').on('click',async () => {
        ...
        const updateResponse = await fetch('/api/stripe/updatePaymentIntent', {
            method: "POST",
            body: formData,
        }).then((response) => {
            if (!response.ok) {
